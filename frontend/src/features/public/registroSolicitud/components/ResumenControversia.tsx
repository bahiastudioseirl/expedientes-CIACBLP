...
interface ResumenControversiaProps {
    tipoResumen: 'textarea' | 'archivo';
    setTipoResumen: (tipo: 'textarea' | 'archivo') => void;
    resumen: string;
        ...
        <FormSection title="Resumen de la controversia" number={3}>
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-start sm:items-center">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            type="radio"
                            name="tipoResumen"
                            checked={tipoResumen === 'textarea'}
                            onChange={() => setTipoResumen('textarea')}
            ...
            {tipoResumen === 'textarea' ? (
                ...
                <div className="space-y-1.5">
                    ...
                    <small className="text-xs text-gray-500">Formatos permitidos: PDF, DOC, DOCX</small>
                </div>
            )}
            {error && (
