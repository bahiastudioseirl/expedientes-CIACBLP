            ...
            <div className="bg-white border-2 border-slate-200 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {solicitud.partes.map(parte => (
                  <div key={parte.id} className={`p-5 rounded-xl border-2 ${
                    parte.tipo === 'demandante' 
                      ? 'bg-emerald-50 border-emerald-200' 
                      : 'bg-rose-50 border-rose-200'
                  }`}>
                    <div className="flex items-center mb-3">
                      <div className={`p-2 rounded-lg ${
                        parte.tipo === 'demandante' ? 'bg-emerald-100' : 'bg-rose-100'
                      }`}>
                        <User className={`w-4 h-4 ${
                          parte.tipo === 'demandante' ? 'text-emerald-600' : 'text-rose-600'
                        }`} />
                      </div>
                      <h4 className={`font-bold text-lg ml-2 capitalize ${
                        parte.tipo === 'demandante' ? 'text-emerald-800' : 'text-rose-800'
                      }`}>
                        {parte.tipo}
                      </h4>
                    </div>
